...
# 加载数据集
dataset = Planetoid(root='C:/Users/kabu/Desktop/data/Citeseer', name='Citeseer')

# GCNConv网络
class GCN(nn.Module):
    def __init__(self, num_node_features, num_classes):
        super(GCN, self).__init__()
        self.conv1 = pyg_nn.GCNConv(num_node_features, 32)
        self.conv2 = pyg_nn.GCNConv(32, num_classes)
...
plt.figure()
lw=2
color = ['orange','blue','green','red','darkorange','navy']
for i in range(n_classes):
    plt.plot(fpr[i],tpr[i],color=color[i],lw=lw,label='ROC for category '+ str(i)+'(AUC=%0.2f)'%roc_auc[i])

plt.plot(fpr['all'],tpr['all'],label='ROC for all category(AUC={0:0.2f})'.format(roc_auc['all']),color='black',linestyle=':',linewidth=4)
plt.xlim([0.0,1.0])
plt.ylim([0.0,1.05])
